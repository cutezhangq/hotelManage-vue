        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-select v-model="query.queryName" placeholder="关键词" class="handle-select mr10">
                    <el-option key="1" label="类别代码" value="type_id"></el-option>
                    <el-option key="2" label="商品类别名称" value="goods_type"></el-option>
            ...
            <el-table
                ...
                <el-table-column prop="type_id" label="类别代码" align="center"></el-table-column>
                <el-table-column prop="goods_type" label="商品类别名称" align="center"></el-table-column>
        ...
        <el-dialog title="新增" :visible.sync="addVisible" width="30%">
            <el-form ref="form" :model="add_form" label-width="70px">
                <el-form-item label="商品类别名称">
                    <el-input v-model="add_form.goods_type"></el-input>
                </el-form-item>
        ...
        <el-dialog title="编辑" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :model="form" label-width="70px">
                <el-form-item label="类别代码">
                    <el-input v-model="form.type_id" :disabled="true"></el-input>
                </el-form-item>
                <el-form-item label="商品类别名称">
...
export default {
    ...
    methods: {
        //获取roomType数据
        getDate(){
          get(`/dao.show_goodsType?index=${this.query.pageIndex}&${this.query.queryName}=${this.query.queryContent}`)
          .then( data =>{
            if(data.code === 200){
        ...
        handleSearch() {
          ...
          get(`/dao.show_goodsType?${query_name}=${query_content}`)
          .then( data =>{
            if(data.code === 200){
              if(data.data.infoList.length > 0){
                 this.tableData = data.data.infoList;
        ...
        //保存新增
        saveAdd(){
          post(`/dao.add_goodsType?goods_type=${this.add_form.goods_type}`+
        ...
        //删除一条数据
        delOneDate(index){
          let cur_id = this.tableData[index].type_id;
        ...
        delAllSelection() {
            ...
            if(length > 0){
              for (let i = 0; i < length; i++) {
                  str += this.multipleSelection[i].goods_type + ' ';
                  strIds += this.multipleSelection[i].type_id + ',';
              }
              //发送批量删除请求
              get(`/dao.del_goodsType?type_id=${strIds}`)
              .then(data =>{
                if(data.code === 200){
        ...
        saveEdit() {
          ...
          post(`/dao.update_goodsType?type_id=${curEdit_row.type_id}`+
                `&goods_type=${curEdit_row.goods_type}`+
