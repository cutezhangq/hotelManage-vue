        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-select v-model="query.queryName" placeholder="关键词" class="handle-select mr10">
                    <el-option key="1" label="楼层ID" value="floor_id"></el-option>
                    <el-option key="2" label="楼层名" value="floor_name"></el-option>
                    <el-option key="3" label="备注" value="details"></el-option>
            ...
            <el-table
                ...
                <el-table-column prop="floor_id" label="楼层ID" align="center"></el-table-column>
                <el-table-column prop="floor_name" label="楼层名" align="center"></el-table-column>
        ...
        <el-dialog title="新增" :visible.sync="addVisible" width="30%">
            <el-form ref="form" :model="add_form" label-width="70px">
                <el-form-item label="楼层名">
                    <el-input v-model="add_form.floor_name"></el-input>
                </el-form-item>
        ...
        <el-dialog title="编辑" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :model="form" label-width="70px">
                <el-form-item label="楼层ID">
                    <el-input v-model="form.floor_id" :disabled="true"></el-input>
                </el-form-item>
                <el-form-item label="楼层名">
...
export default {
    ...
    methods: {
        //获取roomType数据
        getDate(){
          get(`/dao.show_floorInfo?index=${this.query.pageIndex}&${this.query.queryName}=${this.query.queryContent}`)
          .then( data =>{
            if(data.code === 200){
        ...
        handleSearch() {
          ...
          get(`/dao.show_floorInfo?${query_name}=${query_content}`)
          .then( data =>{
            if(data.code === 200){
              if(data.data.infoList.length > 0){
                 this.tableData = data.data.infoList;
        ...
        //保存新增
        saveAdd(){
          post(`/dao.add_floorInfo?floor_name=${this.add_form.floor_name}`+
        ...
        //删除一条数据
        delOneDate(index){
          let cur_id = this.tableData[index].floor_id;
          get(`/dao.del_floorInfo?floor_id=${cur_id}`)
          .then(data =>{
            if(data.code === 200){
              this.$message.error(`删除了${this.tableData[index].floor_name},1条数据`);
        ...
        delAllSelection() {
            ...
            if(length > 0){
              for (let i = 0; i < length; i++) {
                  ...
                  strIds += this.multipleSelection[i].floor_id + ',';
              }
              //发送批量删除请求
              get(`/dao.del_floorInfo?floor_id=${strIds}`)
              .then(data =>{
                if(data.code === 200){
        ...
        saveEdit() {
          ...
          post(`/dao.update_floorInfo?floor_id=${curEdit_row.floor_id}`+
                `&floor_name=${curEdit_row.floor_name}`+
